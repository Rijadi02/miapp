    ...
    @section('content')
        ...
        <script>
            ...
            window.onload = (event) => {
                if (isMobile()) {
                    window.location =
                        "miapp://articles/article?slug={{ $blog->slug }}&title={{ urlencode($blog->title) }}&author={{ urlencode($blog->author) }}&image={{ urlencode($blog->image) }}&date={{ $blog->updated_at->format('Y-m-d') }}&category={{ $blog->category }}&redirect=true";
