    ...
    @section('content')
        ...
        <script>
            ...
            window.onload = (event) => {
                if (isMobile()) {
                    window.location =
                        "miapp://articles/article?slug={{ $blog->slug }}&title={{ encodeURIComponent($blog->title) }}&author={{ encodeURIComponent($blog->author) }}&image={{ encodeURIComponent($blog->image) }}&date={{ $blog->updated_at->format('Y-m-d') }}&category={{ $blog->category }}&redirect=true";
                                    ...
                                    @foreach ($blogs as $bloga)
                                        <li>
                                            <div class="sidebar__post-image">
                                                <img style="aspect-ratio: 0.75; object-fit:cover"
                                                    src="/storage/{{ $bloga->image }}" alt="">
                                            </div>
